...
export const useMessageStore = create<MessageStore>((set, get) => ({
  ...
  setMessage: (message) => {
    if (message?.id === "log-in" || message?.id === "recovery-key" || message?.id === "confirm-email") {
      set({ message: { visible: false, message: null, actionText: null, onPress: () => null, data: {}, icon: "", type: "normal", id: "none" } });
      return;
    }
    set({ message: { ...message } });
  },
