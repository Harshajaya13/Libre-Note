...
import { readFile, rename, stat, writeFile, readdir } from "fs/promises";
...
async function* glob(patterns, opts = {}) {
  ...
  for (const pat of patternArray) {
    const baseDir = pat.replace(/\/\*.*$/, "").replace(/\/\*\*.*$/, "");
    const searchDir = path.resolve(root, baseDir);
    if (!existsSync(searchDir)) continue;
    
    const entries = await readdir(searchDir, { recursive: true, withFileTypes: true }).catch(() => []);
    for (const entry of entries) {
      if (opts.exclude && opts.exclude(entry)) continue;
